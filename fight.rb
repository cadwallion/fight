...
module FightRPG
  ...
  class User
    ...
    def save key, value
      FightRPG.database.set "user:#{@nick}:#{key}", value
    end

    def fetch key
      FightRPG.database.get "user:#{@nick}:#{key}"
    end
  end
end
...
class Fight
  ...
  EQUIPMENT = YAML.load_file(File.dirname(__FILE__) + '/equipment.yml')
  ...
  def initialize bot
    super
    FightRPG.set_database bot.database
  end
  ...
  def create(m)
    user = FightRPG::User.create m.user.nick
    starterweapon = WEAPONS[1][user.weapon_index]
    starterarmor = ARMOR[1][user.armor_index]
    m.reply "-> New character created for 02#{user.nick}. Starting with [06#{starterweapon['name']} | DMG: 131-#{starterweapon['damage']} | ELE: 14#{starterweapon['element']} ] and [06#{starterarmor['name']} | ARM: 050-#{starterarmor['armor']} | ELE: 14#{starterarmor['element']}]"
  end
